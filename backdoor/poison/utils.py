...
from .datasets import cifar, utils as ds_utils
from .datasets import speech
...
def save_module(module: nn.Module, filepath: Path) -> NoReturn:
    ...
    optim, sched = module.optim, module.sched
    torch.save({
                "module-state-dict": module.state_dict(),
                "optim-state-dict": optim.state_dict() if optim is not None else None,
                "sched-state-dict": sched.state_dict() if sched is not None else None,
                }, str(filepath))
...
def load_module(module: nn.Module, filepath: Path):
    ...
    :return: Loaded model
    """
    state_dicts = torch.load(str(filepath), map_location=TORCH_DEVICE)
    ...
    module.eval()
    # Check the optimizer
    optim_state = state_dicts["optim-state-dict"]
    if module.optim is None:
        assert optim_state is None, "No optimizer but optimizer state dict found"
    else:
        module.optim.load_state_dict(optim_state)
    # Check the optimizer scheduler
    sched_state = state_dicts["sched-state-dict"]
    if module.sched is None:
        assert sched_state is None, "No optim shceduler but scheduler state dict found"
    else:
        module.sched.load_state_dict(sched_state)
    return module
...
def construct_filename(prefix: str, out_dir: Path, file_ext: str, ex_id: Optional[int] = None,
    ...
    atk_name = config.BACKDOOR_ATTACK.name.lower().replace('_', '-')
    fields = [config.DATASET.name.lower().replace("_", "-"),
              f"div={config.val_div()}",
              f"t-idx={config.TARG_IDX}",
              f"t-cls={config.ORIG_TARG_CLS}",
              f"p-cls={config.ORIG_POIS_CLS}",
              f"n-p={config.BACKDOOR_CNT}",
              f"n-sep={config.NUM_SUBEPOCH}",
              f"delt={config.BACKDOOR_DELTA}",
              f"atk={atk_name}"]
    if config.has_min_perturb():
        fields.append(f"p-rat={config.MIN_PERTURB_RATIO:.0%}")
    if config.DEBUG:
        fields.append("dbg")
...
def configure_dataset_args() -> Tuple[TensorGroup, LearnerModule]:
    ...
    if config.DATASET.is_cifar():
        ...
        tg = speech.load_data()
        net = speech.build_model(tg.tr_x)
    else:
...
def set_random_seeds(seed: int = 42) -> NoReturn:
    r"""
...
class Batch:
    ...
    def check(self) -> NoReturn:
        ...
        assert self.xs.shape == self.ds.shape, "Feature shape mismatch"
    ...
    def filter_by_lbl(self, keep_lbl: int) -> NoReturn:
        r""" Remove any examples whose label does not match the specified value """
        mask = self.lbls == keep_lbl
        self.lbls = self.lbls[mask]
        self.xs, self.ds = self.xs[mask], self.ds[mask]
        self.bd_ids, self.ds_ids = self.bd_ids[mask], self.ds_ids[mask]
...
class ClassifierBlock(nn.Module):
    ...
    def name(self) -> str:
        ...
        flds = [self._name_prefix] if self._name_prefix else []
        flds.append(self.loss.name())
        if self._n_subepoch > 1:
            flds.append(f"n-sub={self._n_subepoch:03d}")
        return "-".join(flds)
    ...
    def get_epoch_backdoor_ids(self, ep: int) -> Tensor:
        r""" Get the epoch backdoor IDs """
    ...
    def init_fit_vars(self, dl: DeviceDataLoader) -> NoReturn:
    ...
    def calc_valid_loss(self, epoch: int, valid: DeviceDataLoader):
    ...
    @property
    def num_backdoor(self) -> int:
        r""" Accessor for the number of backdoor examples used by the block """
    ...
    def organize_batch(self, batch_tensors: Tuple[Tensor, ...], process_mask: bool,
                       verify_contents: bool = True, include_holdout: bool = False) -> Batch:
        r"""
        ...
        :param include_holdout: If \p True, include the heldout examples in the batch
        :return:
        """
        assert len(batch_tensors) == 6, "Unexpected batch length"
        xs, ds, lbls, adv_lbls, bd_ids, ds_ids = batch_tensors

        if process_mask:
            xs, lbls = xs.clone(), lbls.clone()
            # Mask returns which tensors are adversarially perturbed
            adv_mask = self.loss.build_mask(ids=bd_ids, include_holdout=include_holdout)
            if self.loss.has_any(adv_mask):
                lbls[adv_mask] = adv_lbls[adv_mask]

            # Checks if changes required so return the original tensors
            if config.DATASET.is_speech():
                cl_mask = lbls == adv_lbls
                mask = cl_mask | adv_mask
                xs, lbls = xs[mask], lbls[mask]
                ds = ds[mask]
                bd_ids, ds_ids = bd_ids[mask], ds_ids[mask]
            elif self.loss.has_any(adv_mask):
                # Clone above prevents collision of changes across multiple parallel learners
                xs[adv_mask] += ds[adv_mask]

        batch = Batch(xs=xs, ds=ds, lbls=lbls, bd_ids=bd_ids, ds_ids=ds_ids)
        return batch


def build_min_perturb_mask(ds: Tensor) -> BoolTensor:
    r"""
    Constructs a mask whether each training example meets the perturb threshold

    :param ds: Perturb tensor
    :return: Boolean mask which if \p True corresponding example meets the miniumm perturb value
    """
    assert config.MIN_PERTURB_RATIO is not None, "No perturb ratio specified"
    perturb_threshold = config.MIN_PERTURB_RATIO * config.BACKDOOR_DELTA
    return ds_utils.calc_perturb_dist(ds=ds) >= perturb_threshold
