...
class RiskEstimatorBase(ABC):
    def __init__(self, n_bd: int, train_loss: Callable, valid_loss: Optional[Callable] = None,
        ...
        self.n_bd = n_bd
        assert self.n_bd <= config.get_first_heldout_id(), "Cannot use any heldout backdoors"
    ...
    def build_mask(self, ids: Tensor, include_holdout: bool = False) -> Tensor:
        r""" Construct the examples to which the backdoor will be applied """
        mask = ids < self.n_bd  # type: Tensor
        if include_holdout:
            mask |= influence_utils.get_holdout_mask(bd_ids=ids)
        return mask
...
class Loss(RiskEstimatorBase):
    ...
    def name(self) -> str:
        flds = [f"L-adv={self.n_bd}" if self.is_poisoned() else "L-clean"]
