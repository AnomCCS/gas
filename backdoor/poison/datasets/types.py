...
class PoisonDataset(Enum):
    ...
    FASHION_MNIST = DatasetParams("FashionMNIST", MNIST_DIM, 10)
    KMNIST = DatasetParams("KMNIST", MNIST_DIM, 10)
    MNIST = DatasetParams("MNIST", MNIST_DIM, 10)

    SPEECH = DatasetParams("SPEECH", SPEECH_DIM, 10)
    ...
    def is_mnist(self) -> bool:
        ...
        return self in (self.FASHION_MNIST, self.KMNIST, self.MNIST)

    def is_speech(self) -> bool:
        r""" Returns \p True if dataset is a MNIST or MNIST-variant dataset """
        return self == self.SPEECH


class BackdoorAttack(Enum):
    BLENDING = "blending"
    FOUR_PIXEL = "four-pixel"
    NA = "NA"
    ONE_PIXEL = "one-pixel"
