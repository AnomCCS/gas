...
from .types import BackdoorAttack
...
def _create_raw_images(tg: TensorGroup, labels: Optional[List[str]]) -> NoReturn:
    ...
    run = wandb.init(project=wandb_utils.get_proj_name(), job_type="upload")
    # create an artifact for all the raw data
    # create an artifact for all the raw data
    raw_data_at = wandb.Artifact(str(raw_dir), type="raw_data")

    split_counts = dict()
    for prefix in ("tr", "val", "test"):
        ...
        # d = tg.__getattribute__(f"{prefix}_d").clone()
        for is_adv in (False, True):
            full_prefix = prefix if not is_adv else f"{prefix}_adv"
            # Add perturbation
            if is_adv:
                x += tg.__getattribute__(f"{prefix}_d")
                y = tg.__getattribute__(f"{prefix}_adv_y")

            split_counts[full_prefix] = y.numel()
            # Make the y directory
            cur_dir = raw_dir / full_prefix
            ...
            for idx in range(ids.numel()):
                ...
                # # Write the perturbation vector
                # if not is_adv:
                #     _img_dir = cur_dir / f"{_y_val}_{y_lbl_suffix}_d"
                #     _img_dir.mkdir(exist_ok=True, parents=True)
                #     name = f"d-{ids[idx].item():05d}.png"
                #     path = _img_dir / name
                #     # Write the image file
                #     _save_image(d[idx], path=path)
                #     # add file to artifact by full path
                #     raw_data_at.add_file(str(path), name=str(_img_dir / name))
...
def _split_data():
    ...
    run = wandb.init(project=_wandb.get_proj_name(), job_type="data_split")
...
def _build_proj_dir(prefix: str) -> Path:
    ...
    flds = [prefix]
    if config.BACKDOOR_ATTACK != BackdoorAttack.NA:
