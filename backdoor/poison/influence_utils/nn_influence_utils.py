...
def get_loss_with_weight_decay(device: torch.device, n_gpu: int,
                               model: torch.nn.Module, f_loss: Callable,
                               x: Tensor, y: Tensor, weight_decay: Optional[float],
                               weight_decay_ignores: Optional[List[str]],
                               get_activations: bool = False) \
        -> Union[Tensor, Tuple[Tensor, Tensor]]:

    loss, acts = get_loss_without_wd(device=device, model=model, x=x, y=y, f_loss=f_loss)
...
def get_loss_without_wd(device: torch.device, model: torch.nn.Module, f_loss: Callable,
...
def compute_gradients(device: torch.device, n_gpu: int, model: torch.nn.Module, f_loss: Callable,
                      x: Tensor, y: Tensor,
...
def compute_influences(model: torch.nn.Module, device: torch.device, n_gpu: int,
                       f_loss: Callable, test_dl: torch.utils.data.DataLoader,
    ...
    model64, s_test_flat = model.double(), _flatten(vec=s_test).double()
    for index, train_inputs in enumerate(tqdm(instance_train_data_loader, disable=config.QUIET)):
        ...
        dot_val = -torch.dot(grad_z, s_test_flat).view([1]).double()
        inf_unnormed.append(dot_val)
    ...
    tensors.inf_sim = torch.cat(inf_normed, dim=0).double()
    tensors.inf_sim_l = torch.cat(inf_layer_norm, dim=0).double()
