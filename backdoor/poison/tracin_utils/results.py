...
def export(block: parent_utils.ClassifierBlock, epoch: Optional[int], subepoch: Optional[int],
    ...
    is_bd = influence_utils.label_ids(bd_ids, n_bd=block.num_backdoor)
...
def _store_stats_in_block(block: parent_utils.ClassifierBlock) -> NoReturn:
    ...
    tracin_stats.topk_inf = torch.sum(tracin_inf[:config.BACKDOOR_CNT]).item()
    tracin_stats.botk_inf = torch.sum(tracin_inf[-config.BACKDOOR_CNT:]).item()
...
def generate_epoch_stats(ep: Optional[int], subepoch: Optional[int], method: InfluenceMethod,
    ...
    _, sorted_bd_ids, _ = utils.sort_ids_and_inf(inf_arr=inf_vals, bd_ids_arr=bd_ids,
                                                 ds_ids_arr=ds_ids)
    # Calculate and store the epoch's AUPRC
    influence_utils.calc_backdoor_auprc(block=block, ep=ep, subepoch=subepoch,
                                        bd_ids=bd_ids, ds_ids=ds_ids, inf=inf_vals,
                                        res_type=method, ex_id=ex_id)


def export_combined(block, all_in: bool,
                    full_ds_ids: Tensor, full_bd_ids: Tensor, valid_dl: DeviceDataLoader,
    ...
    # Extract both whether the example was poison and whether that poison was heldout
    bd_labels = influence_utils.label_ids(bd_ids=full_bd_ids)
    is_holdout = influence_utils.label_holdout(bd_ids=full_bd_ids)
    is_valid = label_valid(block=block, full_ds_ids=full_ds_ids, valid_dl=valid_dl)

    assert bd_labels.numel() == is_holdout.numel() == full_ds_ids.numel() == is_valid.numel(), \
        "Num elements mismatch"
    ...
    for file_cnt in range(2):
        ...
        with open(str(filename), "w+") as f_out:
            ...
            f_out.write("id,is_backdoor,is_holdout,is_valid")
            ...
            for row in range(full_ds_ids.numel()):
                ...
                line = [f"{full_ds_ids[row].item()}", f"{bd_labels[row].item()}",
                        f"{is_holdout[row].item()}", f"{is_valid[row].item()}"]
...
def label_valid(block, full_ds_ids: Tensor, valid_dl: DeviceDataLoader) -> Tensor:
