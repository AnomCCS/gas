...
def calc_representer_vals(erm_learners: CombinedLearner, train_dl: DeviceDataLoader,
                          test_x: Tensor, method: InfluenceMethod = InfluenceMethod.REP_POINT,
                          ex_id: Optional[int] = None) -> dict:
    r"""
    Calculates the Representer Point values.

    :param erm_learners:
    :param train_dl:
    :param test_x:
    :param method: Version of repesenter point method to use
    ...
    logging.info(f"{method.value} Execution Time: {total:.3f} seconds")
    return ret


def _calc_representer_vals(erm_learners: CombinedLearner, train_dl: DeviceDataLoader,
                           test_x: Tensor, method: InfluenceMethod = InfluenceMethod.REP_POINT,
                           ex_id: Optional[int] = None) -> dict:
    r"""
    ...
    train_dl = train_dl.new(drop_last=False, shuffle=False, batch_size=1)
    train_dl.dl.dataset.transform = config.get_test_tfms()

    # msg = f"Calculating {method.value} values"
    # if ex_id is not None:
    #     msg += f" for Ex={ex_id}"
    # logging.info(f"Starting: {msg}")

    _calc_alpha_i(erm_learners, train_dl, inc_tr_loss=inc_tr_loss)
...
def _calc_alpha_i(erm_learners: CombinedLearner, train_dl: DeviceDataLoader,
    ...
    for block_name, block in erm_learners.blocks():  # type: str, utils.ClassifierBlock
        ...
        loss = block.loss  # type: RiskEstimatorBase
        # Get parameter values
        f_loss = loss.tr_loss  # Loss function used for calculating gradient
...
def _calc_feature_dot_product(train_dl: DeviceDataLoader, block: utils.ClassifierBlock,
    ...
    for batch_tensors in train_dl:
        ...
        if batch.skip():
            continue
        assert len(batch) == 1, "Only single batches supported"
...
def _error_check_grad(n_rows: int, dec_scores: Tensor, lbls: Tensor) -> Tensor:
    ...
    offset = torch.zeros_like(dec_scores, device=TORCH_DEVICE)
...
def log_representer_scores(idx: int, lbl: int, rep_dict: dict, file_prefix: str,
                           method: InfluenceMethod = InfluenceMethod.REP_POINT,
    ...
    for block_name, (block, rep_vals) in rep_dict.items():  # type: str, (ClassifierBlock, Tensor)
        ...
        influence_utils.calc_backdoor_auprc(block=block, res_type=method, bd_ids=bd_ids,
                                            ds_ids=ds_ids, inf=rep_vals, ex_id=ex_id)
