...
def setup(quiet_mode: bool, log_level: int = logging.DEBUG,
          log_dir: Optional[Union[Path, str]] = None,
          job_id: Optional[Union[int, List[int]]] = None) -> NoReturn:
    r"""
    Logger Configurator
    ...
    # if not quiet_mode and not hasattr(setup, "_HAS_ROOT"):
    #     create_stdout_handler(log_level, format_str=FORMAT_STR)
    #     setup._HAS_ROOT = True
    ...
    if torch.cuda.is_available():
        logging.debug("# CUDA GPUs: %d", torch.cuda.device_count())
...
class TrainingLogger:
    ...
    #
    # @classmethod
    # def create_tensorboard(cls, file_path: Union[Path, str], hparams: OptDict = None):
    ...
    #
    # @classmethod
    # def _check_tensorboard_exists(cls) -> NoReturn:
