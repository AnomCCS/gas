...
import copy
import dataclasses
from typing import Callable, Collection, List, Union, Optional, Tuple
...
def get_loss_with_weight_decay(device: torch.device, n_gpu: int, model: torch.nn.Module,
                               f_loss, x: Tensor, y: Tensor, weight_decay: Optional[float],
...
def compute_gradients(device: torch.device, n_gpu: int, model: torch.nn.Module,
                      ...
                      weight_decay_ignores: Optional[List[str]],
                      create_graph: bool = True, conv_only: bool = False,
    ...
    :param conv_only: If \p True, use only the convolutional backbone not the classification head
    ...
    # # inputs = [param for name, param in model.named_parameters() if name not in params_filter]
    if conv_only:
        if isinstance(model, parent_utils.ClassifierBlock):
            inputs = get_model_params(model=model.get_module().conv_only(),
                                      params_filter=params_filter)
        else:
            inputs = get_model_params(model=model.conv_only(), params_filter=params_filter)
    else:
...
def compute_s_test(model: torch.nn.Module, n_gpu: int, device: torch.device, f_loss: Callable,
                   ...
                   damp: float, scale: float,
                   num_samples: Optional[int] = None) -> List[torch.Tensor]:
    ...
    with tqdm(total=num_samples, disable=config.QUIET) as pbar:
        for i, inputs in enumerate(train_data_loader):
            ...
            this_estim = compute_hessian_vector_products(model=model, n_gpu=n_gpu, device=device,
                                                         f_loss=f_loss, vectors=last_estimate, # noqa
                                                         x=x, y=y, params_filter=params_filter,
                                                         weight_decay=weight_decay,
                                                         weight_decay_ignores=weight_decay_ignores)
...
def compute_influences(model: torch.nn.Module, device: torch.device, n_gpu: int, ids: LongTensor,
                       f_loss: Callable,
                       test_dl: torch.utils.data.DataLoader,
    ...
    if precomputed_s_test is not None:
        ...
        for _ in range(s_test_iterations):
            _s_test = compute_s_test(model=model, n_gpu=n_gpu, device=device, test_dl=test_dl,
                                     f_loss=f_loss,
    ...
    # Use doubles for final product to prevent underflows
    inf_unnormed, inf_normed, inf_layer_norm = [], [], []
    inf_loss_normed, inf_lderiv_normed = [], []
    model64, s_test_flat = copy.deepcopy(model).double(), _flatten(vec=s_test).double()
    for index, train_inputs in enumerate(tqdm(instance_train_data_loader, disable=config.QUIET)):
        ...
        xs, ys = train_inputs[0].double(), train_inputs[1]
        grad_z = compute_gradients(n_gpu=n_gpu, device=device, model=model64, f_loss=f_loss,
                                   x=xs.double(), y=ys, params_filter=params_filter,
                                   weight_decay=weight_decay,
                                   weight_decay_ignores=weight_decay_ignores,
                                   create_graph=False)
        # Need to calculate loss separately since above it includes weight decay
        with torch.no_grad():
            loss, acts = get_loss_without_wd(device=device, model=model64, x=xs, y=ys,
                                             f_loss=f_loss)
            if loss.item() <= 0:
                loss = utils.MIN_LOSS
        lderiv = utils.derivative_of_loss(acts=acts, lbls=ys.to(parent_utils.TORCH_DEVICE),
                                          f_loss=f_loss)
        lderiv.abs_()
        if lderiv.item() <= 0:
            lderiv = utils.MIN_LOSS

        layer_norm = build_layer_norm(grad_z)  # Must call before flatten to keep layer structure
        grad_z = _flatten(grad_z)
        inf_unnormed.append(-torch.dot(grad_z, s_test_flat).view([1]))
        inf_loss_normed.append(inf_unnormed[-1] / loss)
        inf_lderiv_normed.append(inf_unnormed[-1] / lderiv)
        # More numerically stable to normalize first to prevent underflows
        inf_normed.append(-torch.dot(grad_z / grad_z.norm(), s_test_flat).view([1]))
        inf_layer_norm.append(-torch.dot(grad_z / layer_norm, s_test_flat).view([1]))

    # Construct the final tensors
    tensors = InfFuncTensors()
    tensors.ids = ids
    tensors.inf_base = torch.cat(inf_unnormed, dim=0)
    tensors.inf_loss_norm = torch.cat(inf_loss_normed, dim=0)
    tensors.inf_lderiv_norm = torch.cat(inf_lderiv_normed, dim=0)
    tensors.inf_sim = torch.cat(inf_normed, dim=0)
    tensors.inf_sim_l = torch.cat(inf_layer_norm, dim=0)
    tensors.s_test = s_test
    return tensors


def _flatten(vec: Collection[Tensor]) -> Tensor:
    r""" Flatten the gradient into a vector """
    return torch.cat([flat.detach().view([-1]) for flat in vec], dim=0)


def _normalize_hvp(hvp: Collection[Tensor]):
...
def build_layer_norm(grad) -> Tensor:
    ...
    for layer, vec in zip(gbl_layer_norm, grad):  # type: Tensor, Tensor
        ...
        if norm == 0:
            norm = 1E-8
        layer.fill_(norm)
    return _flatten(gbl_layer_norm)
