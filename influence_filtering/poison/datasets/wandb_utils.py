...
def _create_raw_images(tg: TensorGroup, labels: Optional[List[str]]) -> NoReturn:
    ...
    for is_pre in [True, False]:
        for ds_name in ("tr", "val", "test"):
            if is_pre:
                ds_name = f"pre{ds_name}"
            x = tg.__getattribute__(f"{ds_name}_x").clone()
            y = tg.__getattribute__(f"{ds_name}_y").clone()
            ids = tg.__getattribute__(f"{ds_name}_ids").clone()

            split_counts[ds_name] = y.numel()
            # Make the y directory
            cur_dir = raw_dir / ds_name
...
def _build_proj_dir(prefix: str) -> Path:
    ...
    flds = [prefix, f"{config.TARG_CLS}{config.POIS_CLS}"]
