...
def construct_filename(prefix: str, out_dir: Path, file_ext: str, ex_id: Optional[int] = None,
    ...
    if config.TARG_IDX is not None:
        fields.append(f"t-idx={config.TARG_IDX}")
    if config.DEBUG:
        fields.append("dbg")
...
def configure_dataset_args() -> Tuple[TensorGroup, LearnerModule]:
    ...
    if config.DATASET.is_cifar10():
        cifar_dir = dirs.DATA_DIR / "CIFAR10"
        tg = cifar10.load_data(cifar_dir)
    else:
    ...
    return tg, build_init_module(tg=tg)


def build_init_module(tg: TensorGroup) -> LearnerModule:
    r""" Standardizes building the initial module """
    if config.DATASET.is_cifar10():
        net = cifar10.build_model(tg.tr_x)
    else:
        raise ValueError(f"Dataset generation not supported for {config.DATASET.name}")
...
class Batch:
    ...
    def check(self) -> NoReturn:
        ...
        assert self.lbls.shape == self.ids.shape, "IDs tensor does not match shape"
...
class ClassifierBlock(nn.Module):
    def __init__(self, net: nn.Module, estimator: RiskEstimatorBase, n_subepoch: int = 1,
                 name_prefix: str = "", filt_ids: Optional[LongTensor] = None):
        super().__init__()
        if n_subepoch < 1:
        ...
        # Denotes which ID values to keep/remove
        self._keep_mask = None
        if filt_ids is not None:
            assert filt_ids.max().item() + 1 <= config.N_FULL_TR, "Unexpected ID number"
            self._keep_mask = torch.ones(config.N_FULL_TR, device=TORCH_DEVICE, dtype=torch.bool)
            self._keep_mask[filt_ids] = False
    ...
    def _update_subepoch(self, ids: Tensor) -> NoReturn:
        r""" Updates the subepoch information """
        # Stores the ids visited during the epoch in order
        self._ep_ids[-1].append(ids.cpu())
        self._n_ex_cur_epoch += ids.shape[0]
    ...
    def process_batch(self, batch_tensors: Tuple[Tensor, ...]) -> NoReturn:
        ...
        self._update_subepoch(ids=batch.ids)
    ...
    def create_lr_sched(self, lr: float, train_dl: DeviceDataLoader, is_pretrain: bool) -> NoReturn:
    ...
    def organize_batch(self, batch_tensors: Tuple[Tensor, ...],
                       verify_contents: bool = True) -> Batch:
        r"""
        Organizes the tensors from the \p DataLoader into a single \p Batch object container.
        ...
        :return:
        """
        assert len(batch_tensors) == 3, "Unexpected batch length"
        xs, lbls, ids = batch_tensors
        if self._keep_mask is not None:
            assert ids.max() + 1 <= self._keep_mask.shape[0], "ID is larger than mask"
            mask = self._keep_mask[ids]
            xs, lbls, ids = xs[mask], lbls[mask], ids[mask]

        # noinspection PyTypeChecker
        batch = Batch(xs=xs, lbls=lbls, ids=ids)
...
def get_proj_name() -> str:
    ...
    flds = ["inf-filt", config.DATASET.name, f"{config.TARG_CLS}{config.POIS_CLS}"]
