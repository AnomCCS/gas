...
def _combine_and_log_results(block: parent_utils.ClassifierBlock, ep: int, subep: int,
    ...
    # Log test results
    def _log_result(inf_vals: Tensor, method: InfluenceMethod, use_ex_id: bool = True,
                    name: str = "", log_cutoff: bool = False, log_stats: bool = False) -> NoReturn:
        tmp_ex_id = ex_id if use_ex_id else None
        results.generate_epoch_stats(ep=ep, subepoch=subep, block=block, method=method,
                                     inf_vals=inf_vals[full_ds_ids],
                                     ds_ids=full_ds_ids, bd_ids=full_bd_ids,
                                     ex_id=tmp_ex_id, log_cutoff=log_cutoff)
        if name:
            results.export(block=block, epoch=ep, subepoch=subep,
                           vals=inf_vals[full_ds_ids], ds_ids=full_ds_ids, bd_ids=full_bd_ids,
                           res_desc=name, ex_id=tmp_ex_id)
        if log_stats:
            utils.log_vals_stats(block=block, res_type=method, ep=ep, subep=subep,
    ...
    for _inf, _method, _name in utils.get_tracincp_log_flds(tensors=tensors):
        _log_result(_inf, method=_method, name=_name)
    ...
    for _inf, _method, _name in utils.get_gas_log_flds(tensors=tensors):
        _log_result(_inf, method=_method, name=_name)
    ...
    for _inf, _method, _name in utils.get_tracin_log_flds(tensors=tensors):
        _log_result(_inf, method=_method, name=_name)

    # Logs and accessors the adversarial and clean magnitude ratios
    _log_ratio_stats(block=block, ep=ep, subep=subep,
                     full_ds_ids=full_ds_ids, full_bd_ids=full_bd_ids,
                     vals=tensors.subep.loss_vals, ex_id=ex_id, is_grad_norm=False)
    # Logs and accessors the adversarial and clean magnitude ratios
    _log_ratio_stats(block=block, ep=ep, subep=subep,
                     full_ds_ids=full_ds_ids, full_bd_ids=full_bd_ids,
                     vals=tensors.subep.grad_norms, ex_id=ex_id, is_grad_norm=True)
