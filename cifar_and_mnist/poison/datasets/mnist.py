...
def download_mnist_dataset(dest: Path, ds_name: str = "") -> Path:
    ...
    fields = (("KMNIST", torchvision.datasets.KMNIST),
              ("FASHION_MNIST", torchvision.datasets.FashionMNIST),
              ("MNIST", torchvision.datasets.MNIST))
    for name, download_cmd in fields:
        # Dataset must match at beginning of string
        if not re.match(f"^{name}".upper(), ds_name.upper()):
            continue

        download_cmd(root=dest.parent, download=True)
        logging.debug(f"Downloaded dataset {name}")
        dest /= "processed"
        return dest
