...
def calc(block: parent_utils.ClassifierBlock,
    ...
    with ep_tqdm as pbar:
        for ep, ep_wd, ep_bs in pbar:
            tracin_utils.all_in.process_epoch(block32=block,
...
def log_final(block: parent_utils.ClassifierBlock,
              tensors: tracin_utils.TracInTensors, ex_id: Optional[int] = None) -> NoReturn:
    ...
    def _log_result(inf_vals: Tensor, method: InfluenceMethod, name: str = "",
        ...
        if name:
            tracin_utils.results.export(block=block, epoch=None, subepoch=None,
