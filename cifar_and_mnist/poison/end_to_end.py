...
def run(block: utils.ClassifierBlock, tg: TensorGroup,
    ...
    tracin.log_final(block=block, tensors=all_inf, ex_id=config.TARG_IDX)
    # with open(path, "rb") as f_in:
    #     all_inf = pk.load(f_in)

    fields = ((InfluenceMethod.GAS, all_inf.gas_sim),
              (InfluenceMethod.TRACINCP, all_inf.tracincp),
              (InfluenceMethod.TRACIN, all_inf.tracin_inf),
              (InfluenceMethod.TRACIN_SIM, all_inf.tracin_sim),
              )
