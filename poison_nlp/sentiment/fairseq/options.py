...
def parse_args_and_arch(parser, input_args=None, parse_known=False, suppress_defaults=False):
    ...
    if args.arch is not None:
        assert args.archs is None # don't set archs if you have arch
    else:
        assert args.archs is not None # set archs if you don't set arch
        assert args.restore_files is not None # use restore-files not restore-file for attacks
        args.arch = args.archs.split(',')[0]
    ...
    if hasattr(args, 'arch') and args.archs is None:
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    if train:
        ...
        group.add_argument('--poison_example', default=None, type=str)
...
def add_optimization_args(parser):
    ...
    group.add_argument('--lrs', default=None, type=str)
    group.add_argument('--total-num-updates', default=None, type=str)
    group.add_argument('--lr-schedulers', default=None, type=str)
    group.add_argument('--warmup-updatess', default=None, type=str)    
    group.add_argument('--end-to-end-mode', default=False, action='store_true')    
...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--restore-file', type=str, default=None,
                       help='Use for training. filename from which to load checkpoints '
                            '(default: <save-dir>/checkpoint_last.pt')
    group.add_argument('--restore-files', type=str, default=None,
                       help='Use for poisoning. comma separated list of filenames from which to load checkpoints '
                            '(default: <save-dir>/checkpoint_last.pt')    
...
def add_model_args(parser):
    ...
    group.add_argument('--arch', '-a', default=None, metavar='ARCH',
                      choices=ARCH_MODEL_REGISTRY.keys(),
                       help='Model Architecture')
    group.add_argument('--archs', type=str, default=None, help='Set archs when doing poisoning, otherwise use arch. Do not set both')
    # fmt: on
    return group
