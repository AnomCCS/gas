...
def save_checkpoint(args, trainer, epoch_itr, val_loss) -> Tuple[List[int], bool]:
    from . import distributed_utils, meters
    ...
    if args.no_save or not distributed_utils.is_master(args):
        raise ValueError("Distributed checkpoints not currently supported")
        # return
    ...
    # Store whether the current epoch has the best loss
    is_best = val_loss is None or is_better(val_loss, save_checkpoint.best)
    ...
    return orig_checkpoints, is_best


def reset_save_checkpoint_best() -> NoReturn:
    r""" Reset the best loss attribute of the save_checkpoint method """
    # assert hasattr(save_checkpoint, "best"), "Trying to delete non-existant best loss attribute"
    if hasattr(save_checkpoint, "best"):
        delattr(save_checkpoint, "best")
...
def _upgrade_state_dict(state):
    ...
    from . import models, registry, tasks
